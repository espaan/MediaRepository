 ...
 *
 * This is the base repository class for thumb size entities.
 */
class MediaRepository_Entity_Repository_Base_ThumbSize extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['keepAspectRatio'] = (int) FormUtil::getPassedValue('keepAspectRatio', 0, 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MediaRepository_Entity_ThumbSize', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MediaRepository_Entity_ThumbSize', 'tbl')
     ...
     *
     * @return array|MediaRepository_Entity_ThumbSize retrieved data array or MediaRepository_Entity_ThumbSize instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MediaRepository_Entity_ThumbSize instances
     */
    ...
    protected function addSearchFilter($qb, $fragment = '')
    {
        ...
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.width = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.height = \'' . $fragment . '\'';
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numThumbSizes';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MediaRepository_Entity_ThumbSize', 'tbl');
     ...
     * @param int    $excludeId  Id of thumb sizes to exclude (optional).
