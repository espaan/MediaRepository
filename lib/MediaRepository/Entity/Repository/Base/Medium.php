 ...
 *
 * This is the base repository class for medium entities.
 */
class MediaRepository_Entity_Repository_Base_Medium extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['owner'] = (int) FormUtil::getPassedValue('owner', 0, 'GET');
        $parameters['searchterm'] = FormUtil::getPassedValue('searchterm', '', 'GET');
        
        $parameters['showLocation'] = (int) FormUtil::getPassedValue('showLocation', 0, 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MediaRepository_Entity_Medium', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MediaRepository_Entity_Medium', 'tbl')
    ...
    public function updateUserField($userFieldName, $userId, $newUserId)
    {
        ...
        if (empty($userFieldName) || !in_array($userFieldName, array('owner'))) {
            return LogUtil::registerArgsError();
        }
        // check id parameter
        if ($userId == 0 || !is_numeric($userId)
     ...
     *
     * @return array|MediaRepository_Entity_Medium retrieved data array or MediaRepository_Entity_Medium instance
     */
     ...
     *
     * @return MediaRepository_Entity_Medium retrieved instance of MediaRepository_Entity_Medium
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MediaRepository_Entity_Medium instances
     */
    ...
    protected function addSearchFilter($qb, $fragment = '')
    {
        ...
        $where .= 'tbl.owner = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.keywords LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.description LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.description2 LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.dateTaken LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.placeTaken LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.notes LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.license LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.areamap LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.latitude = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.longitude = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.zoomFactor = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.dlcount = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.url = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.mediaHandler LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.freeType = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.fileUpload = \'' . $fragment . '\'';
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numMedia';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MediaRepository_Entity_Medium', 'tbl');
     ...
     * @param int    $excludeId  Id of media to exclude (optional).
    ...
    {
        $qb->leftJoin('tbl.mediaHandlers', 'tblMediaHandlers');
        return $qb;
    }
}
