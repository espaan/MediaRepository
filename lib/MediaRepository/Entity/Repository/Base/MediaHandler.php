 ...
 *
 * This is the base repository class for media handler entities.
 */
class MediaRepository_Entity_Repository_Base_MediaHandler extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['active'] = (int) FormUtil::getPassedValue('active', 0, 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MediaRepository_Entity_MediaHandler', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MediaRepository_Entity_MediaHandler', 'tbl')
     ...
     *
     * @return array|MediaRepository_Entity_MediaHandler retrieved data array or MediaRepository_Entity_MediaHandler instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MediaRepository_Entity_MediaHandler instances
     */
    ...
    protected function addSearchFilter($qb, $fragment = '')
    {
        ...
        $where .= 'tbl.mimeType LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.fileType LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.foundMimeType LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.foundFileType LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.handlerName LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.image = \'' . $fragment . '\'';
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numMediaHandlers';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MediaRepository_Entity_MediaHandler', 'tbl');
     ...
     * @param int    $excludeId  Id of media handlers to exclude (optional).
