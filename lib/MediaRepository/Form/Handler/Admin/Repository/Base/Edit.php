 ...
 */
class MediaRepository_Form_Handler_Admin_Repository_Base_Edit extends MediaRepository_Form_Handler_Admin_Edit
{
    /**
    ...
    public function preInitialize()
    {
        ...
        $this->objectTypeCapital = 'Repository';
        $this->objectTypeLower = 'repository';
        $this->objectTypeLowerMultiple = 'repositories';
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        if ($this->mode == 'edit') {
            ...
            if ($this->hasTemplateId !== true) {
                $entity['files'] = $this->retrieveRelatedObjects('medium', 'files', true);
                $entity['thumbSizes'] = $this->retrieveRelatedObjects('thumbSize', 'thumbsizes', true);
            }
        }
    ...
    {
        // redirect to the list of repositories
        $viewArgs = array('ot' => $this->objectType);
        $url = ModUtil::url($this->name, 'admin', 'view', $viewArgs);
    
        if ($args['commandName'] != 'delete' && !($this->mode == 'create' && $args['commandName'] == 'cancel')) {
            // redirect to the detail page of treated repository
            $url = ModUtil::url($this->name, 'admin', 'display', array('ot' => 'repository', 'id' => $this->idValues['id']));
        }
        return $url;
    }
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'create':
                        $message = $this->__('Done! Repository created.');
                        break;
            case 'update':
                        $message = $this->__('Done! Repository updated.');
                        break;
            case 'delete':
                        $message = $this->__('Done! Repository deleted.');
                        break;
        }
        return $message;
    }
    ...
    public function reassignRelatedObjects()
    {
        ...
        $selectedRelations['files'] = $this->retrieveRelatedObjects('medium', 'medrepMedium_FilesItemList', true, 'POST');
        // reassign the thumb sizes eventually chosen by the user
        $selectedRelations['thumbSizes'] = $this->retrieveRelatedObjects('thumbSize', 'medrepThumbSize_ThumbSizesItemList', true, 'POST');
    ...
    {
        $relatedIds = $this->request->request->get('medrepRepository_ThumbSizesItemList', '');
        if ($this->mode != 'create') {
            // remove all existing references
            foreach ($entity->getThumbSizes() as $relatedItem) {
                $entity->removeThumbSizes($relatedItem);
            }
        }
        if (!empty($relatedIds)) {
            ...
            if (is_array($relatedIds) && count($relatedIds)) {
                $idFields = ModUtil::apiFunc('MediaRepository', 'selection', 'getIdFields', array('ot' => 'thumbSize'));
                ...
                $linkObjects = ModUtil::apiFunc($this->name, 'selection', 'getEntities', array('ot' => 'thumbSize', 'where' => $where));
                if (!is_object($entity->getThumbSizes())) {
                    $entity->setThumbSizes(new Doctrine\Common\Collections\ArrayCollection());
                }
                // create new links
                foreach ($linkObjects as $relatedObject) {
                    if ($entity->getThumbSizes()->contains($relatedObject)) {
                        continue;
                    }
                    $entity->addThumbSizes($relatedObject);
                }
            }
        }
    }
