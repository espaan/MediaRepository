 ...
 */
class MediaRepository_Form_Plugin_Base_ItemSelector extends Zikula_Form_Plugin_TextInput
{
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            PageUtil::addVar('javascript', 'modules/MediaRepository/javascript/finder.js');
            PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('MediaRepository'));
        }
        $firstTime = false;

        if (!SecurityUtil::checkPermission('MediaRepository:' . ucwords($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false; //LogUtil::registerPermissionError();
        }

        $categorisableObjectTypes = array('repository', 'medium');
        $mainCategory = null;
        $catId = 0;
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            $mainCategory = ModUtil::apiFunc('MediaRepository', 'category', 'getMainCat', array('ot' => $this->objectType));
