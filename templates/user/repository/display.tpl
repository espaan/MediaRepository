{* purpose of this template: repositories display view in user area *}
...
<div class="z-frontendcontainer">
    <h2>{$templateTitle|notifyfilters:'mediarepository.filter_hooks.repositories.filter'}{icon id='itemactionstrigger' type='options' size='extrasmall' __alt='Actions' style='display: none' class='z-pointer'}</h2>
...
{if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
    <div class="mediarepositoryRightBox">
        ...
        {if isset($repository.files) && $repository.files ne null}
            {include file='user/medium/include_displayItemListMany.tpl' items=$repository.files}
        {/if}
        ...
        <p class="manageLink">
            ...
            <a href="{modurl modname='MediaRepository' type='user' func='edit' ot='medium' repository="`$repository.id`" returnTo='userDisplayRepository'}" title="{$createTitle}" class="z-icon-es-add">
                {$createTitle}
            </a>
        </p>
        {/if}
        ...
        {if isset($repository.thumbSizes) && $repository.thumbSizes ne null}
            {include file='user/thumbSize/include_displayItemListMany.tpl' items=$repository.thumbSizes}
        {/if}
        ...
        <p class="manageLink">
            ...
            <a href="{modurl modname='MediaRepository' type='user' func='edit' ot='thumbSize' repositories="`$repository.id`" returnTo='userDisplayRepository'}" title="{$createTitle}" class="z-icon-es-add">
                {$createTitle}
            </a>
        </p>
        {/if}
...
{/if}
{include file='user/include_categories_display.tpl' obj=$repository panel=true}
{include file='user/include_standardfields_display.tpl' obj=$repository panel=true}
