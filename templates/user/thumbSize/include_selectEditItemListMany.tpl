...
<li id="{$idPrefixItem}">
    {$item.name}
    <a id="{$idPrefixItem}Edit" href="{modurl modname='MediaRepository' type='user' func='edit' ot='thumbSize' id=$item.id forcelongurl=true}">{$editImage}</a>
     <a id="{$idPrefixItem}Remove" href="javascript:medrepRemoveRelatedItem('{$idPrefix}', '{$item.id}');">{$removeImage}</a>
</li>
{/foreach}
{/if}
</ul>
