{* purpose of this template: thumb sizes display view in user area *}
...
<div class="z-frontendcontainer">
    <h2>{$templateTitle|notifyfilters:'mediarepository.filter_hooks.thumbsizes.filter'}{icon id='itemactionstrigger' type='options' size='extrasmall' __alt='Actions' style='display: none' class='z-pointer'}</h2>
...
{if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
    <div class="mediarepositoryRightBox">
        ...
        {if isset($thumbSize.repositories) && $thumbSize.repositories ne null}
            {include file='user/repository/include_displayItemListMany.tpl' items=$thumbSize.repositories}
        {/if}
        ...
        <p class="manageLink">
            ...
            <a href="{modurl modname='MediaRepository' type='user' func='edit' ot='repository' thumbsizes="`$thumbSize.id`" returnTo='userDisplayThumbSize'}" title="{$createTitle}" class="z-icon-es-add">
                {$createTitle}
            </a>
        </p>
        {/if}
...
{include file='user/include_standardfields_display.tpl' obj=$thumbSize}
