...
<p>
    <label for="{$baseID}_id"{$leftSide}>{gt text='Thumb size'}:</label>
    <select id="{$baseID}_id" name="id" {$rightSide}>
        {foreach item='thumbSize' from=$items}{strip}
            <option value="{$thumbSize.id}"{if $selectedId eq $thumbSize.id} selected="selected"{/if}>
                {$thumbSize.name}
            </option>{/strip}
        {foreachelse}
