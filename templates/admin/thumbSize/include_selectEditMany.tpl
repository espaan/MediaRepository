{* purpose of this template: inclusion template for managing related Thumb sizes in admin area *}
    ...
    <div class="z-formrow">
        <div class="medrepRelationRightSide">
            ...
            <div id="{$idPrefix}AddFields">
                ...
                <a id="{$idPrefix}SelectorDoNew" href="{modurl modname='MediaRepository' type='admin' func='edit' ot='thumbSize'}" title="{gt text='Create new thumb size'}" class="z-button medrepInlineButton">{gt text='Create'}</a>
            </div>
        ...
        <div class="medrepRelationLeftSide">
            {if isset($userSelection.$aliasName) && $userSelection.$aliasName ne ''}
                ...
                {include file='admin/thumbSize/include_selectEditItemListMany.tpl'  items=$userSelection.$aliasName}
            {elseif $mode ne 'create' || isset($relItem.$aliasName)}
                {include file='admin/thumbSize/include_selectEditItemListMany.tpl'  items=$relItem.$aliasName}
            {else}
                {include file='admin/thumbSize/include_selectEditItemListMany.tpl' }
            {/if}
        </div>
        <br style="clear: both" />
    </div>
