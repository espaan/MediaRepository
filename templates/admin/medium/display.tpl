...
<div class="z-admin-content-pagetitle">
    ...
    <h3>{$templateTitle|notifyfilters:'mediarepository.filter_hooks.media.filter'}{icon id='itemactionstrigger' type='options' size='extrasmall' __alt='Actions' style='display: none' class='z-pointer'}</h3>
</div>
...
{if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
    <div class="mediarepositoryRightBox">
        ...
        {if isset($medium.mediaHandlers) && $medium.mediaHandlers ne null}
            {include file='admin/mediaHandler/include_displayItemListMany.tpl' items=$medium.mediaHandlers}
        {/if}
        ...
        <p class="manageLink">
            ...
            <a href="{modurl modname='MediaRepository' type='admin' func='edit' ot='mediaHandler' medium="`$medium.id`" returnTo='adminDisplayMedium'}" title="{$createTitle}" class="z-icon-es-add">
                {$createTitle}
            </a>
        </p>
        {/if}
